...
class Basic_Spyder():
    ...
    def start_scrap(self, part_url_dict, rule_func):
        ...
        result = {key: value for key,
                  value in result.items() if len(value) > 1}
...
official_news = {
    ...
    '国家教育部': {
        ...
        'favicon': 'http://www.moe.gov.cn/favorite.ico',
        'result': gj_edu_result},
...
def huashang():
    ...
    for item in hot.find_all('a'):
        ...
        post_date = article_soup.find(
            'span', class_='article-time').get_text().split(' ')[0]
...
def xinlang():
    ...
    def get_title_date(article_url):
        ...
        try:
            title = article_soup.find('h1').get_text()
            date = format_date(article_soup.find(
                'span', class_='date').get_text())
        except:
            title = article_soup.find(
                'meta', attrs={'property': "og:title"})['content']
            date = article_soup.find('meta', attrs={'property': "article:published_time"})[
                'content'].split('T')[0]
        return (title, date)
